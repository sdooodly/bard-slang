...
export default function Home() {
  ...
  return (
  <main className="flex min-h-screen flex-col items-center p-4 md:p-8 bg-[#101820]">
  <h1 className="text-2xl md:text-4xl font-bold mb-4 text-teal text-center">Bard Slang</h1>

  <div className="w-full max-w-2xl bg-[#18232e] p-4 md:p-6 rounded-lg shadow-xl">
        <textarea
          className="w-full p-2 md:p-3 border border-teal-500 rounded-md focus:ring-teal-500 focus:border-teal-500 text-base md:text-lg bg-[#101820] text-teal resize-none"
          rows={3}
        ...
        <div className="mt-4 flex flex-col md:flex-row md:flex-wrap justify-between items-center space-y-2 md:space-y-0 md:space-x-4">
          <label className="font-medium text-teal mb-2 md:mb-0">Select Style:</label>
            ...
            {styles.map((style) => (
              <label key={style.key} className="inline-flex items-center">
                <input
                  ...
                  disabled={isLoading}
                />
                <span className="ml-2 text-teal">{style.label}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <button
          ...
          className={`mt-6 w-full py-2 md:py-3 rounded-md text-[#101820] font-semibold transition duration-200 ${
            isLoading
  ...
  <footer className="w-full max-w-2xl flex flex-col items-center text-teal fixed bottom-0 left-1/2 -translate-x-1/2 bg-[#101820] p-2 md:p-4 z-50">
        <div className="flex items-center space-x-2">
          ...
          <a href="https://github.com/sdooodly" target="_blank" rel="noopener noreferrer" className="ml-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#14b8a6" viewBox="0 0 24 24">
  ...
  <div className="w-full max-w-2xl mt-4 md:mt-8">
        {error && (
          <p className="bg-red-900 border border-red-400 text-red-200 p-3 rounded mb-4">
            Error: {error}
          </p>
        )}

        {convertedText && (
          <div className="bg-[#18232e] p-6 rounded-lg shadow-xl">
            <h2 className="text-xl font-semibold mb-2 text-teal">Converted Text:</h2>
            <p className="text-teal whitespace-pre-wrap">{convertedText}</p>
          </div>
        )}
      </div>
    </main>
